...
router.get('/pending-cancellations', protect, authorize('admin'), async (req, res) => {
    try {
        ...
        console.log(`📊 Found ${result.recordset.length} pending cancellation requests`);
        ...
        console.error('Error getting pending cancellation requests:', error);
        res.status(500).json({
            success: false,
            message: 'Lỗi khi lấy danh sách yêu cầu hủy gói',
            error: process.env.NODE_ENV === 'development' ? error.message : undefined
        });
    }
});
    ...
    try {
        ...
        try {
            ...
            await transaction.request()
                ...
                .input('RefundAmount', approveRefund ? refundAmount : null)
                .query(`
                    UPDATE CancellationRequests
                    SET Status = 'approved',
                        ProcessedAt = GETDATE(),
            ...
            // If refund is approved, create refund request record
            if (approveRefund && refundAmount > 0) {
                await transaction.request()
                    ...
                    .input('RefundReason', `Hoàn tiền do hủy gói dịch vụ. ${adminNotes || ''}`)
                    .input('Status', 'approved')
                    .input('ProcessedByUserID', req.user.id)
                    .query(`
                        INSERT INTO RefundRequests (UserID, PaymentID, MembershipID, RefundAmount, RefundReason, Status, ProcessedByUserID, ProcessedAt)
                        VALUES (@UserID, @PaymentID, @MembershipID, @RefundAmount, @RefundReason, @Status, @ProcessedByUserID, GETDATE())
                    `);
            }
    ...
    try {
        ...
        try {
            ...
            await transaction.request()
                .input('CancellationRequestID', cancellationId)
                .input('ProcessedByUserID', req.user.id)
                .input('AdminNotes', adminNotes || 'Yêu cầu hủy gói bị từ chối')
                .query(`
                    UPDATE CancellationRequests
                    SET Status = 'rejected',
                        ProcessedAt = GETDATE(),
            ...
            // Restore membership status to active (remove pending_cancellation)
            await transaction.request()
                .input('MembershipID', cancellation.MembershipID)
                .query(`
                    UPDATE UserMemberships
            ...
            await transaction.request()
                ...
                .input('Message', `Yêu cầu hủy gói ${cancellation.PlanName} đã bị từ chối. Lý do: ${adminNotes || 'Không có lý do cụ thể'}. Gói dịch vụ của bạn vẫn tiếp tục hoạt động bình thường.`)
            ...
            res.json({
                success: true,
                message: 'Đã từ chối yêu cầu hủy gói dịch vụ',
                data: cancellation
            });
