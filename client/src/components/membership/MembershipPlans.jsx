...
const MembershipPlans = () => {
    ...
    if (selectedPlan) {
        try {
            // Gửi yêu cầu cập nhật trạng thái thanh toán thành "Đang chờ"
            ...
            // Kiểm tra xem người dùng đã đăng nhập chưa trước khi tiến hành thanh toán
            if (!user || !user.id) {
                notification.error({
                    message: 'Lỗi xác thực',
                    description: 'Bạn phải đăng nhập để mua gói hội viên'
                });
                setCurrentStep(1);
                return;
            }

            console.log('Gửi yêu cầu thanh toán cho kế hoạch:', selectedPlan.PlanID, 'người dùng:', user.id);

            // Xóa trạng thái thành công trước đó để tránh thông báo lặp lại
            dispatch(clearSuccess());

            // Gọi action purchaseMembership để lưu vào cơ sở dữ liệu
            dispatch(purchaseMembership({
                ...
                .then(response => {
                    ...
                    notification.success({
                        message: 'Thanh toán đã được gửi',
                        description: 'Thanh toán của bạn đang được xử lý.'
                    });
                    setPaymentModalVisible(false);

                    // Hiển thị thông báo về trạng thái thanh toán đang chờ xác nhận
                    notification.info({
                        message: 'Đang xử lý',
                        description: 'Gói hội viên của bạn sẽ được kích hoạt khi thanh toán được xác nhận.'
                    });

                    // Log phản hồi thành công
                    console.log('Phản hồi thành công thanh toán:', response);

                    // Cập nhật lại thông tin người dùng để lấy vai trò mới
                    dispatch(getCurrentUser());

                    // Tải lại lịch sử thanh toán mới
                    fetchPaymentHistory();
                })
                .catch(err => {
                    // Xử lý lỗi an toàn
